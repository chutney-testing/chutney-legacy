...
public class KafkaBasicConsumeTaskTest {
    ...
    @Test
    public void should_consume_json_message_as_map() {
        //given
        TestLogger logger = new TestLogger();
        Task task = new KafkaBasicConsumeTask(TARGET_STUB, TOPIC, GROUP, emptyMap(), 1, null, null, logger);
    ...
    @Test
    public void should_consume_and_select_json_message_as_map_from_given_selector() {
        //given
        TestLogger logger = new TestLogger();
        Task task = new KafkaBasicConsumeTask(TARGET_STUB, TOPIC, GROUP, emptyMap(), 1, "$..[?($.headers.header1=='value1' && $.payload.id==\"1122\")]", null, logger);
        MessageListener<String, String> listener = overrideTaskMessageListenerContainer(task);
        listener.onMessage(
        ...
        listener.onMessage(
            buildRecord(FIRST_OFFSET + 2, "KEY2", "{\"value\": \"test message3\", \"id\": \"1133\" }")
        );
    ...
    public void should_respect_given_timeout() {
        //given
        TestLogger logger = new TestLogger();
        Task task = new KafkaBasicConsumeTask(TARGET_STUB, TOPIC, GROUP, emptyMap(), 1, null, "3 sec", logger);
