...
public class EnvironmentService {
    ...
    public Set<Environment> listEnvironments() {
        return environmentRepository.listNames().stream().map(environmentRepository::findByName)
                .collect(Collectors.toSet());
    }

    public Environment createEnvironment(Environment environment)
            throws InvalidEnvironmentNameException, AlreadyExistingEnvironmentException {
        if (environmentRepository.listNames().stream().map(String::toUpperCase).collect(Collectors.toList())
                .contains(environment.name.toUpperCase())) {
    ...
    public void updateEnvironment(String environmentName, Environment newVersion)
        ...
        Environment newEnvironment = Environment.builder().from(previousEnvironment).withName(newVersion.name)
                .withDescription(newVersion.description).build();
    ...
    public Set<Target> listTargets() {
        ...
        List<Target> targetsList = listEnvironments().stream().flatMap(environment -> environment.targets.stream())
    ...
    public void updateTarget(String environmentName, String previousTargetName, Target targetToUpdate)
            throws EnvironmentNotFoundException, TargetNotFoundException {
        Environment environment = environmentRepository.findByName(environmentName);
