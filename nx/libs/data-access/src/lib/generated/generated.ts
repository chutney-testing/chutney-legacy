...
export type ScenarioQuery = (
  ...
  & { scenario?: Maybe<(
    { __typename?: 'Scenario' }
    & Pick<Scenario, 'id' | 'title' | 'description' | 'tags' | 'status' | 'creationDate' | 'executionDate' | 'content' | 'version'>
  )> }
);
...
export type ScenariosQuery = (
  ...
  & { scenarios?: Maybe<Array<Maybe<(
    { __typename?: 'Scenario' }
    & Pick<Scenario, 'id' | 'title' | 'description' | 'tags' | 'creationDate' | 'executionDate' | 'status'>
...
export type StopCampaignMutation = (
  { __typename?: 'Mutation' }
  & Pick<Mutation, 'stopCampaign'>
);
...
export const SaveCampaignDocument = gql`
  ...
  saveCampaign(input: $input) @rest(type: "Campaign", path: "api/ui/campaign/v1", method: "POST") {
    description
    id
    title
  }
...
export const StopCampaignDocument = gql`
    mutation stopCampaign($campaignId: ID!, $executionId: ID!, $bodyBuilder: RestFunction!) {
  stopCampaign(campaignId: $campaignId, executionId: $executionId) @rest(type: "StopCampaignExecution", path: "api/ui/campaign/execution/v1/{args.executionId}/stop", method: "POST", bodyBuilder: $bodyBuilder)
}
    `;
  ...
  export class StopCampaignGQL extends Apollo.Mutation<StopCampaignMutation, StopCampaignMutationVariables> {
...
export const UpdateCampaignDocument = gql`
  ...
  updateCampaign(input: $input) @rest(type: "Campaign", path: "api/ui/campaign/v1", method: "PUT") {
    description
    id
    title
  }
